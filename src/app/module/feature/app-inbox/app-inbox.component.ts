...
@Component({
  ...
  styleUrls: ['app-inbox.component.scss'],
})
export class AppInboxComponent implements OnInit, OnDestroy {

  displayedColumns: string[] = ['select', 'archive', 'date', 'sender', 'subject'];
  dataSource;
  ...
  applyFilter(filterValue: string) {
    const newData = this.ELEMENT_DATA.filter(data => data.sender.includes(filterValue));
  ...
  archiveToggle(event, element) {
    // console.log(event);
    this.emailService.updateArchive(element.id, !element.archive);
    // this.ELEMENT_DATA = this.emailService.getAllEmails();
